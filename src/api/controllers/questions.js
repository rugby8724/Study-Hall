...
exports.postAddQuestion = async (req, res, next) => {
  const {question, topic, section, 
        choiceOne, correctOne, choiceTwo, correctTwo,
        choiceThree, correctThree, choiceFour, correctFour,} = req.body
...
exports.postUpdateQuestion = async (req, res, next) => {
  const {question, topic, section,
    ...
    choice2, correct2, choiceId2, choice3, correct3, choiceId3} = req.body
  ...
  try {
    ...
    for(let i=0; i<choices.length; i++){
      let choiceToUpdate = await db.Choices.findByPk(choices[i][2])
...
exports.getBySection = async (req, res, next) => {
  ...
  try {
    ...
    res.render('section-questions', {questions: sectionQuestions, choices: choices, pageTitle: `Section ${section}`})
  }catch(err){
    console.error(err)
  }
